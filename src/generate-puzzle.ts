...
export function generatePuzzle(): Puzzle {
    ...
    while (calculateSuccessZoneCoverage(puzzle) !== puzzle.successZone.w * puzzle.successZone.h) {
...
function randomPlacing(puzzle: Puzzle): Puzzle {
    ...
    newPuzzle = {
        ...newPuzzle,
        hand: [
            ...newPuzzle.hand.filter(t => t.id !== tetrominoId),
            tetromino
        ]
    };
...
function calculateSuccessZoneCoverage(puzzle: Puzzle): number {
    const placed: Block[] = puzzle.placed
...
function isTetrominoWithinBounds(puzzle: Puzzle, tetromino: Tetromino, placement: Block): boolean {
...
function randomPosition(puzzle: Puzzle): Block {
    const x = randomNumber(puzzle.successZone.x, puzzle.successZone.x + puzzle.successZone.w - 1);
    const y = randomNumber(puzzle.successZone.y, puzzle.successZone.y + puzzle.successZone.h - 1);
    return { x, y, w: 1, h: 1 };
}
